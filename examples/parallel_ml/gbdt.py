...
if __name__ == "__main__":
    ...
    if role == "server":
        ...
        train_data = Dataset.from_file(train_path, neg_y=False, 
                                       rank=cluster.rank, 
                                       total_ranks=cluster.num_workers)
        valid_data = Dataset.from_file(train_path, neg_y=False, 
                                       rank=cluster.rank, 
                                       total_ranks=cluster.num_workers)
        gbdt = GBDT(
            max_depth=6, 
        ...
        X_train, y_train = load_and_slice(train_path, neg_y=False)
        train_data = (X_train, y_train)
        X_valid, y_valid = load_and_slice(valid_path, neg_y=False)
