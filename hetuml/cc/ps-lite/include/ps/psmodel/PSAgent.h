#ifndef __HETU_ML_PS_PSMODEL_PSAGENT_H_
#define __HETU_ML_PS_PSMODEL_PSAGENT_H_
#include "ps/ps.h"
#include <algorithm>
#include <cmath>
#include <sys/time.h>
...
public:
  ...
  /**
   * \brief PSVector: push Dense vals to PS.
   * \param name name of the PSVector
   * \param vals the vals of push vals
   */
  void vecDensePush(const std::string name, Val* vals){
    const std::vector<Key> &keys = _id2key[name];
    const std::vector<int> &lens = _id2length[name];
    std::vector<std::future<int>> ts(keys.size());
    std::vector<SArray<Val>> _piece_val(keys.size());
  ...
  void vecDensePull(const std::string name, Val* vals){
    const std::vector<Key> &keys = _id2key[name];
    const std::vector<int> &lens = _id2length[name];
    std::vector<std::future<int>> ts(keys.size());
    std::vector<SArray<Val>> _piece_val(keys.size());
...
#endif // __HETU_ML_PS_PSMODEL_PSAGENT_H_
