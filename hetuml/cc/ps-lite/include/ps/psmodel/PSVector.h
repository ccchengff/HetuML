#ifndef __HETU_ML_PS_PSMODEL_PSVECTOR_H_
#define __HETU_ML_PS_PSMODEL_PSVECTOR_H_
#include "ps/ps.h"
...
public:
  ...
  void densePull(Val* rets, const int num_vals){
    CHECK_EQ(num_vals, length) << "#PSVector size mismatch in densePull";
    PSAgent<Val>::Get()->vecDensePull(name, rets);
    return;
  }
   ...
   * push the given indices of this vector (default is add)
   */
  void densePush(Val* vals, const int num_vals){
    CHECK_EQ(num_vals, length) << "#PSVector size mismatch in densePush";
    PSAgent<Val>::Get()->vecDensePush(name, vals);
    return;
  }
...
#endif // __HETU_ML_PS_PSMODEL_PSVECTOR_H_
